    ...
    private void convertToGreyscale()
    {
        for (int i = 0; i < imageHeight; i++)
            for (int j = 0; j < imageWidth; j++)
            {
                ...
                int redIntensity = rgbArray[1];
                int greenIntensity = rgbArray[2];
                int blueIntensity = rgbArray[3];
                
                double redWeight = 0.21;
                double greenWeight = 0.72;
                double blueWeight = 0.07;
                
                int weightedRed = (int) (redIntensity * redWeight);
                int weightedGreen = (int) (greenIntensity * greenWeight);
                int weightedBlue = (int) (blueIntensity * blueWeight);
                
                int weightedAverage =
                    weightedRed + weightedGreen + weightedBlue;
                
                rgbArray[1] = weightedAverage;
                rgbArray[2] = weightedAverage;
                rgbArray[3] = weightedAverage;
                //take three ints for R, G, B and put them back into a single int
                pixelArray2D[i][j] = getPixels(rgbArray);
            }
    ...
    private void blur()
    {
        for (int i = 0; i < imageHeight; i++)
            for (int j = 0; j < imageWidth; j++)
            {
                //get three ints for R, G and B
                int maskDimension = 9;
                int offsetRange = (maskDimension - 1) / 2;
                int numberOfSampledPixels = 0;
                int[] totalNeighboringRGBs = new int[3];
                for (
                    int iOffset = -offsetRange;
                    iOffset <= offsetRange;
                    iOffset++)
                {
                    for (
                        int jOffset = -offsetRange;
                        jOffset <= offsetRange;
                        jOffset++)
                    {
                        int newI = i + iOffset;
                        ...
                        if (newI < 0 || newI >= pixelArray2D.length ||
                            newJ < 0 || newJ >= pixelArray2D[0].length)
                        {
                            continue;
                        }

                        int[] rgbArray = getPixelArray(
                            pixelArray2D[newI][newJ]);

                        totalNeighboringRGBs[0] += rgbArray[1];
                        totalNeighboringRGBs[1] += rgbArray[2];
                        totalNeighboringRGBs[2] += rgbArray[3];

                        numberOfSampledPixels++;
                    }
                }
                
                int[] rgbArray = getPixelArray(pixelArray2D[i][j]);
                
                rgbArray[1] = totalNeighboringRGBs[0] / numberOfSampledPixels;
                rgbArray[2] = totalNeighboringRGBs[1] / numberOfSampledPixels;
                rgbArray[3] = totalNeighboringRGBs[2] / numberOfSampledPixels;
                //take three ints for R, G, B and put them back into a single int
                pixelArray2D[i][j] = getPixels(rgbArray);
            }
